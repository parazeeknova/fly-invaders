"""Declared neurotransmitter-sign proxy used in graph preparation (from DOOMFLY)."""
import numpy as np


def transmitter_signs(transmitters, ambiguous_sign=1):
    """Coarse fast-transmission sign: ACh +; GABA, glutamate, histamine -.

    A co-transmitter combination with only one fast sign uses that sign; conflicting,
    missing and modulator-only cells use `ambiguous_sign`. Never deletes unknown edges.
    """
